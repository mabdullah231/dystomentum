  ...
  return (
    <aside style={{ top: titleBarHeight }} className={`fixed bottom-0 left-0 flex flex-col border-r px-2 py-4 transition-[width,background-color,border-color,box-shadow] duration-300 lg:z-20 lg:w-60 lg:px-3 lg:py-5 ${isCollapsed ? 'z-20 w-16' : 'z-40 w-60 shadow-2xl shadow-black/40'} ${isLightTheme ? 'border-[#D4D4D8] bg-[#FFFFFF]' : 'border-[#1E1E22] bg-[#121215]'}`}>
      <Button
        ...
        className={`mb-6 h-10 w-full border px-0 lg:h-auto lg:px-4 ${!isCollapsed && 'px-4'} ${isLightTheme ? 'border-[#D4D4D8] bg-white text-[#18181B] hover:bg-[#F4F4F5]' : 'border-[#27272A] bg-white text-[#0A0A0C] hover:bg-[#E4E4E7]'}`}
        onClick={() => onNavigate('New Transaction')}
      >
        <Plus className="h-4 w-4" />
        <span className={`${isCollapsed ? 'hidden' : 'inline'} lg:inline`}>New Transaction</span>
      </Button>
      <nav className="sidebar-scroll space-y-1 overflow-y-auto pr-1" aria-label="Main navigation">
        {navigation.map((item) => (
